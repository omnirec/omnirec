...
fn run_highlight(x: i32, y: i32, width: i32, height: i32) -> Result<(), String> {
    ...
    if state.layer_shell.is_none() {
        return Err(
            "Compositor does not support wlr-layer-shell protocol. Highlight not available."
                .to_string(),
        );
    }
    ...
    let (target_output, rel_x, rel_y, scale) =
        if let Some(output_info) = state.find_output_for_point(x, y) {
            // Convert to output-relative coordinates
            let rel_x = x - output_info.x;
            let rel_y = y - output_info.y;
            (
                Some(output_info.output.clone()),
                rel_x,
                rel_y,
                output_info.scale,
            )
